...
comparandola con le densità raggiunte da altre entità come in figura \refbf{fig:powerDensityVSCriticalDim}.
\begin{figure}[H]
...
%come gia menzianto %in \refbf{chIntro:},
...
%sparsity defs
La proprietà di sparsità della matrice $A$  nel sistema \refbf{eq:1}, ovvero la proprietà di avere per la maggioranza degli elementi il valore zero,
...
Sfruttare la sparsità delle matrici nella risoluzione di un problema come \refbf{eq:1}, consente di ottenere svariati vantaggi
...
Solutori di sistemi come \refbf{eq:1}, possono essere di tipo diretto o iterativo.\\
...
come mostrato dalla figura \refbf{fig:Efficient_Linear_System_Solvers_for_Mesh_Processing_densificationByDirectSolver}.\\
\begin{figure}[H]
  \centering \includegraphics[height=.33\textheight,keepaspectratio]{Efficient_Linear_System_Solvers_for_Mesh_Processing_densificationByDirectSolverSimplied.svg.png}
  ...
  \caption[perdità di sparsità nelle matrici fattorizzate da solutori diretti]
	{Varie fattorizzazioni di una matrice altamente sparsa A 500x500 con soli 3502 elementi \nnz (in alto)
				in  matrici con un numero di \nnz incrementato a vari livelli  \citebf{sparseLinearSolverTR}:
				\begin{itemize}
...
può essere riutilizzata per sistemi in cui viene cambiato il lato destro di \refbf{eq:1},
%o anche in caso in cui la matrice $A$ cambia i valori \nnz ma non il loro pattern di sparsità... 50% del tempo
come analizzato in \citebf{sparseLinearSolverTR}.\\
\voidLine
...
un costo per iterazione approssimatamente costante all'incrementare della dimensione del problema { \bf \citebf{AMGgeneralANY,Sp3MM4AMG}}.\\
%MultiGrid in generale-wikipedia
L'idea principale dei metodi MultiGrid è quella di 
risolvere il sistema \refbf{eq:1} iterativamente, applicando una serie di correzioni 
...
%In figura \refbf{fig:Multigrid_Visualization_wikipedia} è disponibile una rappresentazione schemativa di un algoritmo MultiGrid iterativo con un ciclo-V.
\begin{figure}[H]
...
%AMG finally!
I metodi Algebrici riescono ad utilizzare unicamente informazioni derivanti dalla matrice associata al sistema da risolvere \refbf{eq:1},
...
\end{equation}
dove $A_0 = A$ \citebf{AMG4PSBLAS} e $P_l$ è una sequenza di matrici di prolungamento $n_l \times n_{l+1}$ con $n_0=n$ e $n_{l+1}<n_l$.\\
%TODO more?
\par\null\par
...
in parallelo è l'obbiettivo di questa tesi, al fine di potenziare progetti come \citebf{AMG4PSBLAS} che sfruttano solutori basati su AMG.\\
...
ma anche a problemi di ottimizzazione non lineare come problemi di controllo ottimo \citebf{PSBLAS3man}.\\
AMG4PSBLAS \citebf{AMG4PSBLASman} fornisce i metodi MultiGrid di tipo algebrico precedentemente descritti,

...
\citebf{sysReviewChi}.\\ Le principali formulazioni di SpMM sono:
\voidLine
%\begin{itemize}
...
grafi a 3 strati U, W, V {\bf \citebf{2dNewIdeas,cohen3LayeredGraphs}} dove:
\begin{itemize}
...
%e unire i contributi di coppie di vertici che condividono nodi adiacenti\\ %TODO
%%INNER PRODUCT
\begin{figure}[H]
  \centering \includegraphics[width=.6\linewidth,keepaspectratio]{layeredGraphInnerProduct.svg.pdf} 
...
\end{figure}
Per la formulazione Inner-product %\refbf{fig:layeredGraphInnerProduct} 
...
%%OUTER PRODUCT
\begin{figure}[H]
  \centering \includegraphics[width=.6\linewidth,keepaspectratio]{layeredGraphOuterProduct.svg.pdf} 
...
\end{figure}
Per la formulazione Outer-product %\refbf{fig:layeredGraphOuterProduct}
...
%%row-by-row
\begin{figure}[H]
  \centering \includegraphics[width=.6\linewidth,keepaspectratio]{layeredGraphRowByRow.svg.pdf} 
...
\end{figure}
Per la formulazione Row-by-row %\refbf{fig:layeredGraphRowByRow}
...
(per quanto siano disponibili costrutti per una uscita anticipata, che però richiedono una speciale configurazione, disabilitata di default per 
 motivi di performance \citebf{openmp5.1} )\\
%TODO REMOVE...
...
Un UpperBound sulla lunghezza di una riga risultante a SpMM è computabile come riportato nell'algoritmo 4 di \citebf{sysReviewChi}.\\
...
%TODO ad esempio come riportato in \refbf{RITROVA ARTICOLO CHE DESCRIVE IL PRODOTTO SIMBOLICO}

\section{Algoritmi}
%%SRC STUDIES = Gustavson
Molte delle ricerche riguardo SpMM sono basate sull'algoritmo di Gustavson \citebf{gustavson},
un algoritmo sequenziale basato sulla formulazione Row-by-row, riportate in forma di pseudo-codice 
in \refbf{figCode:gustavsonRigheSysSurvey} e graficamente in \refbf{fig:gustavsonRigheGraphicalIntel}.\\

\begin{figure}[H]
  \caption[pseudocodice Gustavson]{pseudo-codice dell'algoritmo di Gustavson}
  \centering \includegraphics[width=.7\linewidth,keepaspectratio]{gustavsonRigheSysSurvey.svg.pdf} \decoRule
...
\begin{figure}[H]
  ...
  \centering \includegraphics[width=.7\linewidth,keepaspectratio]{gustavsonRigheGraphicalIntel.svg.pdf} \decoRule
...
funzionalità sono analizzati successivamente in \refbf{ssec:gustavsonDerivate} %TODO FIX REF
\\
...
\begin{figure}[H]
  \centering \includegraphics[width=.7\linewidth,keepaspectratio]{pMM_cube.svg.pdf} 
...
è data dal cubo di lavoro W (in figura \refbf{fig:pMM_cube})
in cui ogni moltiplicazione di elementi \nnz $a_{i,k}*b_{k,j}$ (minima operazione assegnabile) 
è rappresentabile con voxel $W(i,j,k)$ \citebf{cartesianPartitioningModels}.
...
inferiore alla sua dimensione più grande N \citebf{2dNewIdeas} \\ %TODO newIdeas 14.2.2 x matrici quadrate
...
verrà assegnato ad una matrice di dimensione circa $(n/\sqrt{p})\times(n/\sqrt{p})$.\\ %TODO ~~
...
{\small {\bf D}ouble{\bf C}ompressed{\bf S}parse{\bf C}olumns} (visualizzata in figura \refbf{fig:DCSCvsCSC})
...
\begin{figure}[H]
  \centering \includegraphics[width=.7\linewidth,keepaspectratio]{DCSCvsCSC.svg.pdf} 
  \caption[DCSC vs CSC]{confronto delle rappresentazioni DCSC e CSC}
  \decoRule \label{fig:DCSCvsCSC}
\end{figure}

\subsection{Moltiplicazione tra matrici ipersparse DCSC}
\label{ssec:hypersparseMM}
Segue la descrizione di un algoritmo risolutivo per la moltiplicazione tra matrici
ipersparse \citebf{2dNewIdeas}, basato su una formulazione outer-product ed
utilizzato in alcuni algoritmi SpMM.\\
lo pseudo-codice dell'algoritmo è riportato in \refbf{figCode:hypersparseMM} 
\begin{figure}[H]
  \centering \includegraphics[width=.7\linewidth,keepaspectratio]{hypersparseMM.svg.pdf}
...
\\
Successivamente vengono effettuati $|Isect|$ prodotti cartesiani \refbf{fig:hypersparseMMGraphical} 
...
\begin{figure}[H]
  \centering \includegraphics[width=.7\linewidth,keepaspectratio]{hypersparseMMGraphical.svg.pdf}
...
descritta in \citebf{intelSpMMDenseAccumulator}, sfruttando la
...
in figura \refbf{fig:gustavsonRigheBlocksGraphicalIntel}\\
%Partizionamento = Gustavson a blocchi
...
\citebf{intelSpMMDenseAccumulator} %TODO SOURCE ARTICLE OF ESTIMATE
...
\begin{figure}[H]
	\centering
  		\includegraphics[width=.40\textwidth,keepaspectratio]{gustavsonRigheBlocksIntel.svg.pdf}
  		\includegraphics[width=.50\textwidth,keepaspectratio]{gustavsonRigheBlocksGraphicalIntel.svg.pdf} 
...
affrontato originariamente da \citebf{SPA_mathlab}.\\

\subsubsection{Dense SUMMA} %versione base C=AB
Un partizionamento bidimensionale della risoluzione del prodotto tra matrici
dense è realizzato dell'algoritmo parallelo SUMMA \citebf{denseSumma} che è alla
base di una sua controparte per matrici sparse \citebf{sparseSUMMA}.\\
...
si può eseguire l'operazione di SpMM in parallelo come riportato dallo pseudocodice \refbf{figCode:denseSUMMA}.\\
\begin{figure}[H]
  \centering \includegraphics[width=.333\linewidth,keepaspectratio]{denseSUMMA.svg.pdf}
...
dall'algoritmo Sparse SUMMA \citebf{sparseSUMMA}, derivato dalla sua
controparte per matrici dense.\\
%SPARSE SUMMA
È riportato lo pseudo-codice dell'algoritmo sparseSUMMA 
in \refbf{figCode:sparseSUMMA} e una sua iterazione graficamente in
\refbf{fig:sparseSUMMAIteration}.\\
...
utilizzando l'algoritmo hypersparseMM \refbf{figCode:hypersparseMM}.\\
...
\begin{figure}[H]
  \centering \includegraphics[width=.7\linewidth,keepaspectratio]{sparseSUMMA.svg.pdf} 
  \caption[pseudocodice sparseSUMMA] {pseudocodice dell'algoritmo SparseSUMMA}
  \decoRule \label{figCode:sparseSUMMA}
\end{figure}
\begin{figure}[H]
  \centering \includegraphics[width=.7\linewidth,keepaspectratio]{sparseSUMMAIteration.svg.pdf}
...
algoritmo Split-3D-SpMM \citebf{Split3DSpMM}, dove al partizionamento
...
riportato in figura \refbf{fig:workCube3D}, dove il processo P(i,j,k) possiede la
porzione della matrice di A:\\ 
$A(im/p_r:(i+1)m/p_r - 1, jn/p_c + kn/(p_c p_l ) : jn/p_c + (k + 1)n/(p_c p_l) - 1)$  \\
\begin{figure}[H]
  \centering \includegraphics[width=.7\linewidth,keepaspectratio]{workCube3D.svg.pdf}
...
\end{figure}
L'algoritmo è riportato nello pseudo-codice \refbf{figCode:Split3DSpMM} e una
sua iterazione è raffigurata graficamente in \refbf{fig:Split3DSpMMIteration}.\\

In maniera simile al caso di sparseSUMMA %TODO \refbf{sparseSummaDescriptionStart}
...
\begin{figure}[H] 
  \centering 
  \includegraphics[width=.7\linewidth,keepaspectratio]{Split3DSpMM.svg.pdf} 
...
\end{figure}
\begin{figure}[H]
  \centering \includegraphics[width=.7\linewidth,keepaspectratio]{Split3DSpMMIteration.svg.png} 
...
%%OuterProduct visual
\begin{figure}[H]
  \centering \includegraphics[width=.7\linewidth,keepaspectratio]{imgs/outerProductExampleVisual_ESC.svg.pdf}
...
Una soluzione per l'operazione di SpMM mediante Outer-Product è riportata in \citebf{ESC},
...
%%Algo Steps bigPic
\begin{figure}[H]
  \centering \includegraphics[width=.7\linewidth,keepaspectratio]{OuterProduct_PBAlgoritmExample_ESC.svg.pdf}
...
Analogamente a quanto descritto precedentemente in \refbf{ChExistingTecqs:openMP_for_philosophy}, 
...
Seguendo un approccio Outer-Product per SpMM, il numero di \nnz al termine della fase Expand è pari ad:	$ \sum\limits_{i = 1}^k  | I^i(A) | *| I_i(B) | $ \\ % flops + = nnz(B(i, :)) × nnz(A(:, i))
...
\begin{figure}[H]
  \centering \includegraphics[width=.7\linewidth,keepaspectratio]{OuterProduct_PBAlgo_BinsExample_ESC.svg.pdf}

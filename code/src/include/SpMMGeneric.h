...
///aux structures
//hold SPGEMM result over a unpartitionated space among threads-row[s' blocks]
typedef struct{
...
#endif //SPGEMM_GENERIC_H  	////-> end multiImplementation common part
 ...
 */
SP3GEMM CAT(sp3gemmRowByRowPair_,OFF_F);
 ...
 */
SP3GEMM CAT(sp3gemmRowByRowMerged_,OFF_F);
 ...
 */
SPGEMM CAT(spgemmRowByRow2DBlocksAllocated_,OFF_F);

///implementation wrappers as static array of function pointers
//sp3gemm as pair of spgemm
static SPGEMM_INTERF  CAT(SpgemmFuncs_,OFF_F)[] = {
    & CAT(spgemmRowByRow_,OFF_F),
    & CAT(spgemmRowByRow1DBlocks_,OFF_F),
    & CAT(spgemmRowByRow2DBlocks_,OFF_F),
    & CAT(spgemmRowByRow2DBlocksAllocated_,OFF_F)
};
//sp3gemm as pair of spgemm
static SP3GEMM_INTERF CAT(Sp3gemmFuncs_,OFF_F)[] = {
    & CAT(sp3gemmRowByRowMerged_,OFF_F)
};
